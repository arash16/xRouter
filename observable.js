...
define(['durandal/system', 'durandal/binder', 'knockout'], function (system, binder, ko) {
    ...
    function createLookup(obj, lookup) {
        var value = lookup || {}; // TODO >> publish to github
    ...
    function convertObject(obj, hasChanged) {
        ...
        if (lookup)
            if (!obj.hasOwnProperty('__observable__'))
                lookup = createLookup(obj, Object.create(lookup));

            else if (lookup.__full__)
                return;
        ...
        if (system.isArray(obj)) {
            ...
            for (var propertyName in obj) if (obj.hasOwnProperty(propertyName)) { // TODO >> publish to github
